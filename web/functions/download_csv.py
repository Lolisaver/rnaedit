from ..models import Sample, Level, Site
...
def download_csv(request):
    ...
    if request.GET["aa_change_field"] != "any":
        criterias['utr__iexact'] = request.GET["aa_change_field"]
        file_name_list.append(request.GET["aa_change_field"])
    if any(criterias):
        editing_site_module_list = Site.objects.filter(**criterias).order_by("chromosome", "site")
    ...
    if "button" in request.GET:
        ...
        if request.GET["button"] == "editing_level":
            ...
            #從資料庫將此點的Level全部抓出來
            editing_level_module_list = list(Level.objects.filter(site__chromosome__iexact=request.GET["chr"]).filter(site__site__iexact=request.GET["site"]))
            #將資料轉換成list利前端操作
