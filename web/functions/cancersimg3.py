...
def cancersimg3(request, cancer, rd, lr):
    ...
    if Cancer.objects.get(abbr=cancer).normalSample == 0:
        ...
        pn[0] += '_N'
        p1 = pt.append(pn)
        p1.columns = ['cancer', 'edin', 'is_tumor']
        #v_type = 'combine'
        v_type = 'split'
    p1_name = '/var/www/rnaedit/static/csv/cancers/img3/d%s.csv' % (rd)
    p1['edin'] = p1['edin'].astype(float)
    p1['edin'] -= np.random.rand(p1.shape[0]) / 1000
    p1.to_csv(p1_name, index=False)
    pic = p1_name.split('static/')[1]
    return render(request, "embedviolin.html", {'pic': pic, 'img_src': img_src, 
                    't5': t5_details, 'n5': n5_details, 'v_type': v_type})
