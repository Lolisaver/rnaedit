...
def cancersimg2(request, cancer, rd):
    fig, ax = plt.subplots()
    fig.set_size_inches((16, 8))
    ax.set_ylim(-0.1, 1.1)
    if Cancer.objects.get(abbr=cancer).normalSample == 0:
        timeit()
        t5_details = SET.objects.filter(cancer=cancer).order_by('-p50', '-edin5')[:10]
        ...
        timeit()
        t5_details = SET.objects.filter(cancer=cancer).order_by('-p50', '-edin5')[:5]
        t5 = list(t5_details.values_list('site', flat=True))
        timeit('check1')
        ...
        n5_details = SEN.objects.filter(cancer=cancer).order_by('-p50', '-edin5')[:5]
    ...
    for patch in box['medians']:
        patch.set_color('red')
    fig1 = ax.get_figure()
    p1_name = '/var/www/rnaedit/static/img/cancers/img2/p%s.png' % (rd)
    fig1.savefig(p1_name)
    timeit('check save')
    pic = p1_name.split('static/')[1]
    return render(request, "embedpic.html", {'pic': pic, 'img_src': 'p2img', 't5': t5_details, 'n5': n5_details})
